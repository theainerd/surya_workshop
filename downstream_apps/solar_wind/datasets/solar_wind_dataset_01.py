        ...
        **kwargs,
        ...
        if ds_time_tolerance is not None:
            ...
            if len(self.df_valid_indices) == 0:
                surya_min = min(self.valid_indices)
                surya_max = max(self.valid_indices)
                ds_min = self.ds_index["ds_index"].min()
                ds_max = self.ds_index["ds_index"].max()
                raise ValueError(
                    "No intersection between Surya and DS indices within "
                    f"ds_time_tolerance={ds_time_tolerance!r}.\n"
                    f"Surya index ({ds_index_path and 'this split'}) covers "
                    f"{surya_min} .. {surya_max}.\n"
                    f"DS index ({ds_index_path}) covers {ds_min} .. {ds_max}.\n"
                    "These ranges do not overlap (or overlap only outside the tolerance), "
                    "which usually means the DS index was built for a period the Surya "
                    "split has no frames for (e.g. a data-gap year) rather than a config bug."
                )
